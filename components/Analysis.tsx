...
export const Analysis: React.FC<Props> = ({ result, results, onHome, onPracticeWeakness, tier = 'free', onUpgrade }) => {
  ...
  // Fade-out overlay with upgrade CTA for free users — shows content then fades to prompt
  const FadeUpgrade = ({ gradientFrom }: { gradientFrom?: string }) => (
    <div className="relative">
      <div className={`absolute -top-16 left-0 right-0 h-16 bg-gradient-to-t ${gradientFrom || 'from-white'} to-transparent pointer-events-none z-10`} />
      <div className={`pt-4 pb-2 text-center ${gradientFrom === 'from-slate-900' ? 'bg-slate-900' : 'bg-white'}`}>
        {onUpgrade && (
          <button onClick={onUpgrade} className="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg">
            <Lock size={12} /> Upgrade to See Full
          </button>
        )}
      </div>
    </div>
  ...
  // Helper: truncate text to roughly half
  const halfText = (text: string | undefined) => {
    if (!text) return '';
    const mid = Math.ceil(text.length / 2);
    return text.substring(0, mid) + '...';
  };

  // Helper: truncate HTML to roughly half (by plain text length, preserving tags up to cutoff)
  const halfHtml = (html: string | undefined) => {
    if (!html) return '';
    // Strip tags to get text length
    const textLen = html.replace(/<[^>]*>/g, '').length;
    const targetLen = Math.ceil(textLen / 2);
    let count = 0;
    let cutIndex = 0;
    let inTag = false;
    for (let i = 0; i < html.length; i++) {
      if (html[i] === '<') { inTag = true; }
        ...
        count++;
        if (count >= targetLen) { cutIndex = i + 1; break; }
      }
    }
    if (cutIndex === 0) return html;
    // Close any open tags roughly
    ...
    <div className="min-h-screen w-full max-w-7xl mx-auto p-4 md:p-10 space-y-8 animate-fade-in pb-32">
      ...
      {isFree && (
        <div className="bg-gradient-to-r from-indigo-600 to-violet-600 rounded-[2rem] p-6 md:p-8 text-white text-center space-y-3">
          ...
          <h3 className="text-lg font-black">Preview Mode</h3>
          <p className="text-indigo-100 text-sm max-w-md mx-auto">
            You're seeing a preview of your report. Upgrade to unlock the full analysis for every section.
          </p>
          {onUpgrade && (
            <button
              onClick={onUpgrade}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 items-start">
        ...
        <div className="lg:col-span-4 space-y-6">
          {/* Performance Matrix — show real scores, half for free (1 of 3 visible) */}
          ...
          {/* Expert Perspective — show half text for free */}
          <div className="bg-white border border-slate-200 rounded-[2rem] p-6 md:p-8 shadow-sm space-y-6">
            ...
            <p className="text-slate-600 text-xs md:text-sm leading-relaxed italic">
              "{isFree ? halfText(analysis.expertCritique) : analysis.expertCritique}"
            </p>
            {isFree && analysis.expertCritique && (
              <FadeUpgrade />
            )}
          </div>

          {/* Strategic Framework — show name + explanation, half steps for free */}
          <div className="bg-white border border-slate-200 rounded-[2rem] p-6 md:p-8 shadow-sm space-y-6">
            <div className="flex items-center gap-2 text-indigo-600">
              <Compass size={20} />
            ...
            {analysis.answerFramework ? (
              <div className="space-y-4">
                  ...
                  <div className="space-y-3 pt-2">
                      {(isFree
                        ? analysis.answerFramework.steps.slice(0, Math.ceil(analysis.answerFramework.steps.length / 2))
                        : analysis.answerFramework.steps
                      ).map((s: any, i: number) => (
                          <div key={i} className="flex gap-3">
                              <div className="flex flex-col items-center gap-1">
                                  ...
                                  {i < (isFree ? Math.ceil(analysis.answerFramework.steps.length / 2) : analysis.answerFramework.steps.length) - 1 && <div className="w-px h-full bg-slate-100 my-1"></div>}
                              </div>
                              <div className="pb-2">
                  ...
                  {isFree && analysis.answerFramework.steps.length > 1 && (
                    <FadeUpgrade />
                  )}
              </div>
            ) : (
        ...
        <div className="lg:col-span-8 space-y-6 md:space-y-8">
          {/* Question + Logic Roadmap — show half roadmap steps for free */}
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  {analysis.demoLogicRoadmap && analysis.demoLogicRoadmap.length > 0 ? (
                    (isFree
                      ? analysis.demoLogicRoadmap.slice(0, Math.ceil(analysis.demoLogicRoadmap.length / 2))
                      : analysis.demoLogicRoadmap
                      ...
                      <React.Fragment key={i}>
                        ...
                        {i < (isFree ? Math.ceil(analysis.demoLogicRoadmap.length / 2) : analysis.demoLogicRoadmap.length) - 1 && <ChevronRight className="text-indigo-300" size={16} />}
                      </React.Fragment>
                    ))
                  ) : (
                    <p className="text-xs text-indigo-200 italic">No logic roadmap available.</p>
                  )}
                  {isFree && analysis.demoLogicRoadmap && analysis.demoLogicRoadmap.length > 1 && (
                    <>
                      <ChevronRight className="text-indigo-300" size={16} />
                      <div className="bg-white/10 border border-white/20 px-3 py-2 md:px-5 md:py-2.5 rounded-2xl text-[10px] md:text-xs font-bold backdrop-blur-md opacity-40 flex items-center gap-1.5">
                        <Lock size={10} /> +{analysis.demoLogicRoadmap.length - Math.ceil(analysis.demoLogicRoadmap.length / 2)} more
                      </div>
                    </>
                  )}
                </div>
          ...
          {/* CEO Answer — show half text for free */}
          <div className="bg-slate-900 border border-slate-800 rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-2xl relative group">
            ...
            <div className="p-6 md:p-10 relative z-10">
              <div
                ...
                dangerouslySetInnerHTML={{ __html: isFree ? halfHtml(analysis.ceoDemoAnswer || "Executive insight unavailable.") : (analysis.ceoDemoAnswer || "Executive insight unavailable.") }}
              />
              {isFree && (
                <FadeUpgrade gradientFrom="from-slate-900" />
              )}
            </div>
            <div className="absolute -top-10 -left-10 w-40 h-40 bg-amber-500/20 rounded-full blur-3xl"></div>
          </div>

          {/* Standard Demo Answer — show half text for free, lock improvement path */}
          <div className="bg-white border border-slate-200 rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-sm flex flex-col">
            ...
            <div className="p-6 md:p-10 space-y-6">
              <div
                ...
                dangerouslySetInnerHTML={{ __html: isFree
                  ? halfHtml((analysis.demoAnswer || "").replace(/<u>/g, '<span class="bg-indigo-50 text-indigo-700 font-black px-1.5 rounded-md border-b-2 border-indigo-200">').replace(/<\/u>/g, '</span>'))
                  : (analysis.demoAnswer || "").replace(/<u>/g, '<span class="bg-indigo-50 text-indigo-700 font-black px-1.5 rounded-md border-b-2 border-indigo-200">').replace(/<\/u>/g, '</span>')
                }}
              />
              {isFree ? (
                <FadeUpgrade />
                ...
                <div className="pt-6 border-t border-slate-100">
                  ...
                  <ul className="space-y-3">
                    {analysis.logicUpgrades && analysis.logicUpgrades.length > 0 ? (
                      analysis.logicUpgrades.map((tip: string, i: number) => (
          ...
          {/* Transcript — fully visible for free */}
